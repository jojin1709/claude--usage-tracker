...
class TokenCounter {
	...
	estimateFileTokens(fileMetadata) {
		if (fileMetadata.file_kind === "image") {
			// Anthropic uses a grid system. For 3.5/4.0 models:
			// 1 token per 750 pixels is a rough average, but it's more accurate to use a base + scaled dimensions.
			// Standard large image is ~1600 tokens.
			const width = fileMetadata.preview_asset.image_width || 1024;
			const height = fileMetadata.preview_asset.image_height || 1024;
			
			// Simple grid estimation: (width/750) * (height/750) is too low.
			// Better: Max of (1600) or scaled.
			const pixels = width * height;
			if (pixels > 1_000_000) return 1600;
			return Math.max(300, Math.round(pixels / 1000));
		} else if (fileMetadata.file_kind === "document") {
			// Pages vary wildly, but 1500-2000 is a better average than 2250.
			return 1800 * (fileMetadata.document_asset.page_count || 1);
		}
		return 500; // Base fallback
	}
