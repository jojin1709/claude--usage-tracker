...
class LengthUI {
	...
	createTooltips() {
		...
		return {
			...
			estimate: create('Estimated number of messages of the current size you can send before hitting your next limit.'),
		};
	}
	...
	renderEstimate() {
		...
		if (limiting && limiting.messagesLeft > 0) {
			...
			// Update tooltip with limiting factor info
			const limitNames = { session: 'Session', weekly: 'Weekly', sonnetWeekly: 'Sonnet Weekly', opusWeekly: 'Opus Weekly' };
			const limitName = limitNames[limiting.key] || limiting.key;
			this.elements.tooltips.estimate.textContent = `You have approximately ${estimateValue} messages left.\n\nLimiting factor: ${limitName}\n(Based on current conversation length and model)`;
			return;
		}

		estimate.innerHTML = `${msgPrefix}<span>N/A</span>`;
		this.elements.tooltips.estimate.textContent = 'Unable to estimate remaining messages. Check if you are logged in and usage data is available.';
	}
