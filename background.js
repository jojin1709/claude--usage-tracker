...
async function updateBadge(usageData) {
	if (!CONFIG.SHOW_BADGE || isElectron || !browser.action) return;
	...
	const percentage = Math.round(session.percentage);
	const text = percentage >= 100 ? 'MAX' : `${percentage}%`;
	const color = percentage >= (CONFIG.WARNING_THRESHOLD * 100) ? '#de2929' : '#2c84db';
...
async function updateAllTabsWithUsage(usageData = null) {
	...
	// Update badge with the first available usage data (usually the primary org)
	if (usageData) {
		updateBadge(usageData);
	} else if (tabs.length > 0) {
		const firstTab = tabs[0];
		const orgId = await requestActiveOrgId(firstTab);
		const api = new ClaudeAPI(firstTab.cookieStoreId, orgId);
